...
from data.DOCData2 import textline_ext
...
class registUnetBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        mov = (input[:, :1] + 1) / 2#20230403改 经过数据增强的图片复原 wropdoc数据
        textline = (input[:,1:2] + 1) /2
        mask = input[:,2:3]
          
        out = self.spatialtransform(mov, flow)
        mask_rect = self.spatialtransform(mask, flow)
        # textline = self.textline_extractor(out)
        textline_rect = self.spatialtransform(textline, flow)
        
        return out, flow, textline_rect, mask_rect
    
    
class RectNet(nn.Module):
    def __init__(self, input_nc, encoder_nc, decoder_nc): 
        super(RectNet, self).__init__() 
        self.coarse = registUnetBlock(input_nc,encoder_nc,decoder_nc)
        self.refine = registUnetBlock(input_nc,encoder_nc,decoder_nc)
           
    def forward(self, x):
        rect1, flow1, textline1, mask1 = self.coarse(x)
        rect2, flow2, textline2, mask2 = self.refine(torch.cat([rect1,textline1,mask1],dim=1))
        return rect2, flow2, textline2, mask2